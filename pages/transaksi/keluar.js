...
export default function BarangKeluar() {
  ...
  const fetchRecent = async () => {
    ...
    const res = await fetch('/api/transaksi?tipe=keluar&limit=10', {
  ...
  return (
    <Layout>
      <div className="page-header">
        ...
        <p className="page-subtitle">Input pengeluaran barang dari gudang</p>
      </div>

      <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
        <div style={{ flex: '0 0 auto' }}>
          <TransaksiForm tipe="keluar" onSuccess={fetchRecent} />
        </div>

        <div style={{ flex: 1, minWidth: 300 }}>
          <div className="card">
            <div style={{ fontWeight: 500, fontSize: 14, marginBottom: 16 }}>Keluar Terbaru</div>
                ...
                <tbody>
                  {recent.length === 0 ? (
                    ...
                    <tr key={t.id}>
                      ...
                      <td><span style={{ color: 'var(--danger)', fontWeight: 500 }}>-{t.jumlah}</span> {t.satuan}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
